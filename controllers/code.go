...
func (self *CodeController) List() {
	self.Data["pageTitle"] = "Configurações do código de status"
	self.display()
}

func (self *CodeController) Add() {
	self.Data["pageTitle"] = "Adicionar código de status"
	self.display()
}

func (self *CodeController) Edit() {
	self.Data["pageTitle"] = "editar código de status"
...
func (self *CodeController) AjaxSave() {
	...
	if Code_id == 0 {
		...
		_, err := models.CodeGetByName(Code.Code)
		///fmt.Println(res)
		if err == nil {
			self.ajaxMsg("O código de status já existe", MSG_ERR)
		}
