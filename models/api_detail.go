...
func ApiTreeData(groupId int) ([]*ApiSourceList, error) {
	...
	sql := "SELECT id,source_name FROM api_source WHERE group_id=?"
	orm.NewOrm().Raw(sql, groupId).QueryRows(&list)
	apiList := make([]*ApiSourceList, 0)
	for _, apisource := range list {
		detailList := make([]*ApiList, 0)
		detail_sql := "SELECT id,method,api_name FROM api_detail WHERE source_id=? AND status=3"
...
func ApiFullDetailById(id int) (detail *ApiDetails, err error) {
	sql := "SELECT api_detail.*,a.real_name as create_name,b.real_name as update_name,c.real_name as audit_name FROM api_detail LEFT JOIN uc_admin as a ON api_detail.create_id=a.id  LEFT JOIN uc_admin as b ON api_detail.update_id=b.id  LEFT JOIN uc_admin as c ON api_detail.audit_id=c.id  WHERE api_detail.id=?"
...
func ApiChangeStatus(ids string, status int) (num int64, err error) {
	...
	sql := "UPDATE api_detail set status=? where id in (" + ids + ")"
