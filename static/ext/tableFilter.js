  ...
  var mod = {
    ...
    render: function (myTable) {
      ...
      if (!(myTable.filter && typeof myTable.filter.bottom !== 'undefined' && !myTable.filter.bottom) && $table.next().children('.soul-bottom-contion').length === 0) {
        $table.next().children('.layui-table-box').after('<div class="soul-bottom-contion"><div class="condition-items"></div><div class="editCondtion"><a class="layui-btn layui-btn-primary">Editar filtros</a></div></div>')
      ...
      if ($('#soul-filter-list' + tableId).length === 0) {
        ...
        var soulFilterList = [],
          filterItemsHtml = {
            column: '<li class="soul-column"><i class="layui-icon layui-icon-table"></i> coluna da tabela <i class="layui-icon layui-icon-right" style="float: right"></i></li>',
            data: '<li class="soul-dropList"><i class="soul-icon soul-icon-drop-list"></i> filtrar dados <i class="layui-icon layui-icon-right" style="float: right"></i></li>',
            condition: '<li class="soul-condition"><i class="soul-icon soul-icon-query"></i> filtro <i class="layui-icon layui-icon-right" style="float: right"></i></li>',
            editCondition: '<li class="soul-edit-condition"><i class="layui-icon layui-icon-edit"></i> Editar filtros </li>',
            excel: '<li class="soul-export"><i class="soul-icon soul-icon-download"></i> Exportar para Excel </li>',
            clearCache: '<li class="soul-clear-cache"><i class="layui-icon layui-icon-delete"></i> Limpar cache </li>'
          };
        ...
        soulFilterList.push('<li class="soul-sort" data-value="asc" ><i class="soul-icon soul-icon-asc"></i> Ascendente </li>');
        soulFilterList.push('<li class="soul-sort" data-value="desc"  style="border-bottom: 1px solid #e6e6e6"><i class="soul-icon soul-icon-desc"></i> ordenação descendente </li>');
          ...
          if (_this.startsWith(filterType, 'date')) {
            ...
            if (filterSo && filterSo.children && filterSo.children.length > 0) {
              for (i = 0; i < filterSo.children.length; i++) {
                ...
                if (i === 0) {
                  ...
                  conditionHtml.push(
                    '<td>' +
                    '   <div>' +
                    '      <input type="checkbox" name="switch" lay-filter="soul-coondition-switch" lay-skin="switch" lay-text="com | ou" ' + (!prefix || prefix === 'and' ? 'checked' : '') + '>' +
                    '    </div>' +
                    '</td>')
                }
                ...
                conditionHtml.push('<td style="width: 110px;"><div class="layui-block" ><input class="layui-input value" value="' + (value || '') + '" placeholder="valor" /></div></td>');
              ...
              conditionHtml.push('<tr data-id="" data-type="eq" data-value="">'
                ...
                + '</div></td>'
                + '<td style="width: 110px;"><div class="layui-block" ><input class="layui-input value" placeholder="valor" /></div></td>'
                + '<td><i class="layui-icon layui-icon-delete del" style="font-size: 23px; color: #FF5722; cursor: pointer"></i></td>'
                + '</tr>');
            }
            conditionHtml.push('</tbody></table><div style="text-align: center; padding-top: 5px"><button class="layui-btn layui-btn-sm" data-type="add"><i class="layui-icon">&#xe654;</i>adicionar à</button><span style="display: inline-block;width: 50px"></span><button class="layui-btn layui-btn-sm" data-type="search"><i class="layui-icon">&#xe615;</i>Investigar</button></div>')
            ...
            $('#soul-condition' + tableId + ' button[data-type]').on('click', function () {
              ...
              if ($(this).data('type') === 'add') {
                ...
                var newTr = '<tr data-id="' + newId + '"><td>' +
                  '   <div>' +
                  '      <input type="checkbox" name="switch" lay-filter="soul-coondition-switch" lay-skin="switch" lay-text="com | ou" checked>' +
                  '   </div>' +
                  '</td>'
        ...
        $('#main-list' + tableId + ' .soul-clear-cache').on('mouseover', function (e) {
          ...
          layer.msg('Restaurado!', {icon: 1, time: 1000})
        });
              ...
              success: function (result) {
                ...
                for (var key in result) {
                  ...
                  if (!((list.length === 1 && list[0] === '') || list.length === 0)) {
                    ...
                    uls.push("<ul class='" + key + "DropList' data-value='" + key + "'><li style='color: gray;line-height: 25px;padding-left: 20px;'>(sem dados)</li></ul>")
                  }
                }
      ...
      function groupHtml(filterSo, filterBoard, fieldMap, isFirst, isLast) {
        ...
        switch (mode) {
          case 'in':
            ...
            filterBoard.push('<div class="layui-deeppink item-type" >filtrar dados</div>');
            filterBoard.push('<div class="layui-blueviolet item-value">comum' + (filterSo.values ? filterSo.values.length : 0) + 'Dados do artigo</div>');
            filterBoard.push('<div class="layui-red delete-item"><i class="layui-icon layui-icon-close-fill"></i></div>');
            break;
          case 'date':
            filterBoard.push('<div class="layui-firebrick item-field">' + (fieldMap[field].title) + '</div>');
            filterBoard.push('<div class="layui-deeppink item-type">Selecione a data</div>');
            filterBoard.push('<div class="layui-blueviolet item-value">' + (filterSo.type === 'specific' ? filterSo.value || 'por favor escolha' : dateTimeItems[filterSo.type]) + '</div>');
            filterBoard.push('<div class="layui-red delete-item"><i class="layui-icon layui-icon-close-fill"></i></div>');
            break;
          case 'condition':
            filterBoard.push('<div class="layui-firebrick item-field">' + (fieldMap[field].title) + '</div>');
            ...
            if (type !== 'null' && type !== 'notNull') {
              filterBoard.push('<div class="layui-blueviolet item-value">' + (typeof filterSo.value === 'undefined' || filterSo.value === '' ? 'Por favor, insira...' : filterSo.value) + '</div>');
            }
            filterBoard.push('<div class="layui-red delete-item"><i class="layui-icon layui-icon-close-fill"></i></div>');
            break;
          case 'group':
            filterBoard.push('<div class="layui-firebrick">agrupamento</div>')
            filterBoard.push('<div><a class="layui-btn layui-btn-xs" data-type="addOne"><i class="layui-icon layui-icon-add-1"></i> Adicionar condição</a><a class="layui-btn layui-btn-xs" data-type="addGroup"><i class="layui-icon layui-icon-add-circle"></i> Adicionar grupo</a></div>')
            filterBoard.push('<div class="layui-red delete-item"><i class="layui-icon layui-icon-close-fill"></i></div>');
        ...
        $.ajax({
          ...
          success: function (res) {
            ...
            if (res[myTable.response.statusName] != myTable.response.statusCode) {
              layer.msg('Os dados retornados não estão de acordo com a especificação, o código de status de sucesso correto deve ser:"' + myTable.response.statusName + '": ' + myTable.response.statusCode, {
                icon: 2,
                anim: 6
              });
            } else {
          ...
          error: function (res) {
            layer.msg('Solicitar exceção!', {icon: 2, anim: 6});
            ajaxStatus = false;
          }
        })
        if (!ajaxStatus) {
