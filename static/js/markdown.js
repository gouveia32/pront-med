$(function () {
    ...
    window.editor = editormd("docEditor", {
        ...
        toolbar : true,
        placeholder: "本Editar器支持MarkdownEditar，左边编写，右边预览",
        imageUpload: true,
           ...
           $("#documentTemplateModal").modal("show");
            ...
            if(Object.prototype.toString.call(window.documentCategory) === '[object Array]' && window.documentCategory.length > 0){
                if($("#markdown-save").hasClass('change')) {
                    var comfirm_result = confirm("Editar内容未保存，需要保存吗？")
                    if(comfirm_result) {
                        saveDocument();
                    }
                }
        ...
        $.ajax({
            ...
            success : function (res) {
                ...
                if(res.errcode === 0){
                    ...
                    var confirmIndex = layer.confirm('Confirma a alteração?', {
                        btn: ['Sim','Não'] //按钮
                    }, function(){
    ...
    $("#addDocumentForm").ajaxForm({
        beforeSubmit : function () {
            ...
            if (doc_name === ""){
                return showError("O nome do diretório não pode ficar vazio","#add-error-message")
            }
    ...
    $("#sidebar").jstree({
        ...
        if($("#markdown-save").hasClass('change')) {
            if(confirm("O conteúdo for alterado e não é salvo, deseja salvá-lo?")){
