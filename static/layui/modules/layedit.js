...
layui.define(['layer', 'form'], function(exports){
          ...
          upload.render({
            ...
            ,done: function(res){
              if(res.code == 0){
                ...
                layer.msg(res.msg||'falha no upload');
              }
            }
          });
        });
  ...
  ,link = function(options, callback){
    var body = this, index = layer.open({
      ...
      ,content: ['<ul class="layui-form" style="margin: 15px;">'
        ...
        ,'</li>'
        ,'<li class="layui-form-item">'
          ,'<label class="layui-form-label" style="width: 60px;">caminho aberto</label>'
          ,'<div class="layui-input-block" style="margin-left: 90px">'
            ,'<input type="radio" name="target" value="_self" class="layui-input" title="janela atual"'
            + ((options.target==='_self' || !options.target) ? 'checked' : '') +'>'
            ,'<input type="radio" name="target" value="_blank" class="layui-input" title="nova janela" '
            + (options.target==='_blank' ? 'checked' : '') +'>'
          ,'</div>'
        ,'</li>'
        ,'<li class="layui-form-item" style="text-align: center;">'
          ,'<button type="button" lay-submit lay-filter="layedit-link-yes" class="layui-btn"> sim </button>'
          ,'<button style="margin-left: 20px;" type="button" class="layui-btn layui-btn-primary"> não </button>'
        ,'</li>'
      ,'</ul>'].join('')
  ...
  ,face = function(callback){
    //表情库
    var faces = function(){
      var alt = ["[sorrir]", "[嘻嘻]", "[Ha ha]", "[可爱]", "[可怜]", "[cutucar o nariz]", "[surpreenda-se]", "[tímido]", "[挤眼]", "[闭嘴]", "[鄙视]", "[爱你]", "[泪]", "[偷笑]", "[亲亲]", "[生病]", "[太开心]", "[白眼]", "[右哼哼]", "[左哼哼]", "[嘘]", "[衰]", "[委屈]", "[吐]", "[哈欠]", "[抱抱]", "[怒]", "[疑问]", "[馋嘴]", "[拜拜]", "[思考]", "[汗]", "[困]", "[睡]", "[钱]", "[失望]", "[酷]", "[色]", "[哼]", "[鼓掌]", "[晕]", "[悲伤]", "[抓狂]", "[黑线]", "[阴险]", "[怒骂]", "[互粉]", "[心]", "[伤心]", "[猪头]", "[熊猫]", "[兔子]", "[ok]", "[耶]", "[good]", "[NO]", "[赞]", "[来]", "[弱]", "[草泥马]", "[神马]", "[囧]", "[浮云]", "[给力]", "[围观]", "[威武]", "[奥特曼]", "[礼物]", "[钟]", "[话筒]", "[蜡烛]", "[蛋糕]"], arr = {};
  ...
  ,code = function(callback){
    var body = this, index = layer.open({
      ...
      ,content: ['<ul class="layui-form layui-form-pane" style="margin: 15px;">'
        ,'<li class="layui-form-item">'
          ,'<label class="layui-form-label">Por favor, selecione o idioma</label>'
        ...
        ,'</li>'
        ,'<li class="layui-form-item" style="text-align: center;">'
          ,'<button type="button" lay-submit lay-filter="layedit-code-yes" class="layui-btn"> sim </button>'
          ,'<button style="margin-left: 20px;" type="button" class="layui-btn layui-btn-primary"> não </button>'
        ,'</li>'
      ,'</ul>'].join('')
      ,success: function(layero, index){
  ...
  ,tools = {
    ...
    ,strong: '<i class="layui-icon layedit-tool-b" title="negrito" lay-command="Bold" layedit-event="b"">&#xe62b;</i>'
    ,italic: '<i class="layui-icon layedit-tool-i" title="itálico" lay-command="italic" layedit-event="i"">&#xe644;</i>'
    ,underline: '<i class="layui-icon layedit-tool-u" title="下划线" lay-command="underline" layedit-event="u"">&#xe646;</i>'
    ,del: '<i class="layui-icon layedit-tool-d" title="tachado" lay-command="strikeThrough" layedit-event="d"">&#xe64f;</i>'
