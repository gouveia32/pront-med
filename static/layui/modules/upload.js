...
layui.define('layer' , function(exports){
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,ajaxSend = function(){
      ...
      layui.each(items, function(index, file){
        ...
        var opts = {
          ...
          ,error: function(){
            aborted++;
            that.msg('Falha ao tentar faze upload.');
            error(index);
            allDone();
          }
        };
    ...
    ,iframeSend = function(){
      ...
      Class.timer = setInterval(function() {
        ...
        try {
          ...
          that.msg('Ocorre uma exceção ao obter as informações de resposta após o upload');
          clearInterval(Class.timer);
          error();
        }
        if(res){
    ...
    ,done = function(index, res){
      ...
      if(typeof res !== 'object'){
        try {
          ...
          res = {};
          return that.msg('Por favor, retorne um JSON válido para a interface de upload');
        }
      }
    ...
    switch(options.accept){
      case 'file': //一般文件
        if(exts && !RegExp('\\w\\.('+ exts +')$', 'i').test(escape(value))){
          that.msg('O arquivo selecionado contém um formato não suportado');
          return elemFile.value = '';
        }
      break;
      case 'video': //视频文件
        if(!RegExp('\\w\\.('+ (exts || 'avi|mp4|wma|rmvb|rm|flash|3gp|flv') +')$', 'i').test(escape(value))){
          that.msg('O vídeo selecionado contém um formato não suportado');
          return elemFile.value = '';
        }
      break;
      case 'audio': //音频文件
        if(!RegExp('\\w\\.('+ (exts || 'mp3|wav|mid') +')$', 'i').test(escape(value))){
          that.msg('O áudio selecionado contém um formato não suportado');
          return elemFile.value = '';
        }
      break;
      default: //图片文件
        ...
        if(check){
          that.msg('A imagem selecionada contém um formato não suportado');
          return elemFile.value = '';
        }
      break;
    }
    ...
    if(options.number && that.fileLength > options.number){
      return that.msg('O número máximo que pode ser carregado ao mesmo tempo é:'+ options.number);
    }
    ...
    if(options.size > 0 && !(device.ie && device.ie < 10)){
      ...
      if(limitSize) return that.msg('arquivo não pode exceder '+ limitSize);
    }
    send();
  };
